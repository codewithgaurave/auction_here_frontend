...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res?.admin?.token) throw new Error("Invalid response from server");

      // save full admin object (adminId, name, token)
      setLoginData(res.admin);
    ...
    >
      ...
      >
        <div className="text-center mb-8">
          <div className="w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center bg-white p-2">
            <img src="aclogo.png" alt="Auction Here Logo" className="w-full h-full object-contain" />
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="adminId" className="block mb-2 text-sm font-medium" style={{ color: themeColors.text }}>
              Admin ID
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="password" className="block mb-2 text-sm font-medium" style={{ color: themeColors.text }}>
              Password
            </label>
            <input
              type="password"
